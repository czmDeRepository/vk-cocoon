...
| `VK_NETWORK_MODE` | `IPv4NAT64` | Guest network contract: `IPv4NAT64` preserves the legacy IPv4 guest path; `IPv6Only` configures Linux/Windows guests for native IPv6, DHCPv6/DNS64, and the corresponding readiness behavior. Invalid values abort startup. |
